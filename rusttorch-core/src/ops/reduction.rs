...
pub fn max(tensor: &Tensor) -> f64 {
    match tensor.data() {
        ...
        TensorData::Int32(arr) => {
            arr.iter().copied().max().unwrap_or(i32::MIN) as f64
        }
        TensorData::Int64(arr) => {
            arr.iter().copied().max().unwrap_or(i64::MIN) as f64
        }
    }
}
...
pub fn min(tensor: &Tensor) -> f64 {
    match tensor.data() {
        ...
        TensorData::Int32(arr) => {
            arr.iter().copied().min().unwrap_or(i32::MAX) as f64
        }
        TensorData::Int64(arr) => {
            arr.iter().copied().min().unwrap_or(i64::MAX) as f64
        }
    }
}
...
pub fn sum_dim(tensor: &Tensor, dim: usize) -> Tensor {
    ...
    match tensor.data() {
        TensorData::Float32(arr) => {
            ...
            Tensor::from_data(TensorData::Float32(result), DType::Float32)
        }
        TensorData::Float64(arr) => {
            let result = arr.sum_axis(ndarray::Axis(dim));
            Tensor::from_data(TensorData::Float64(result), DType::Float64)
        }
        TensorData::Int32(arr) => {
            let result = arr.sum_axis(ndarray::Axis(dim));
            Tensor::from_data(TensorData::Int32(result), DType::Int32)
        }
        TensorData::Int64(arr) => {
            let result = arr.sum_axis(ndarray::Axis(dim));
            Tensor::from_data(TensorData::Int64(result), DType::Int64)
        }
    }
}

/// Mean along a specific dimension
pub fn mean_dim(tensor: &Tensor, dim: usize) -> Tensor {
    assert!(dim < tensor.ndim(), "Dimension out of range");
