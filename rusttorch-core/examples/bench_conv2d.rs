...
fn main() {
    ...
    let input_b1 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 1 * 3 * 32 * 32], &[1, 3, 32, 32]),
        true,
    );
    let input_b8 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 8 * 3 * 32 * 32], &[8, 3, 32, 32]),
        true,
    );
    let input_b32 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 32 * 3 * 32 * 32], &[32, 3, 32, 32]),
        true,
    );
    ...
    let input_deep_b1 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 1 * 64 * 16 * 16], &[1, 64, 16, 16]),
        true,
    );
    let input_deep_b8 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 8 * 64 * 16 * 16], &[8, 64, 16, 16]),
        true,
    );

    bench_forward(
        "Conv2d(64→128, 3x3) batch=1 fwd",
        &conv2,
        &input_deep_b1,
        20,
    );
    ...
    bench_forward_backward(
        "Conv2d(64→128, 3x3) batch=1 fwd+bwd",
        &conv2,
        &input_deep_b1,
        10,
    );
    ...
    let mnist_b32 = Variable::new(
        Tensor::from_vec(vec![0.5f32; 32 * 1 * 28 * 28], &[32, 1, 28, 28]),
        true,
    );
    bench_forward("Conv2d(1→8, 5x5) batch=32 fwd", &conv_mnist, &mnist_b32, 50);
    bench_forward_backward(
        "Conv2d(1→8, 5x5) batch=32 fwd+bwd",
        &conv_mnist,
        &mnist_b32,
        20,
    );
